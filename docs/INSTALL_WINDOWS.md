...
대시보드의 자동 선정 모드에서 "Fable 종목 선정"을 켜면(기본 켜짐), 장이
열릴 때 순위 상위 후보 목록을 Claude Fable에게 보내 매매 종목을 고르게 한다.
선정 근거는 대시보드의 Fable 분석 패널에 표시된다. AI 백엔드가 없거나 선정에
실패하면 기존처럼 순위 상위 종목으로 자동 대체되므로 매매가 막히지 않는다.
선정은 참고 자료이며 투자 자문이 아니다.
